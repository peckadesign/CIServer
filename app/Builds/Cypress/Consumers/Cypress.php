...
namespace CI\Builds\Cypress\Consumers;

class Cypress implements \Kdyby\RabbitMq\IConsumer
{
	...
	/**
	 * @var \CI\Builds\Cypress\StatusPublicator
	 */
	...
	public function __construct(
		...
		\CI\Builds\Cypress\StatusPublicator $statusPublicator,
	...
	public function process(\PhpAmqpLib\Message\AMQPMessage $message)
	{
		...
		$this->logger->addInfo(
			sprintf(
				'Spouští se Cypress pro repozitář "%s" a commit "%s"',
				$repository->name,
		...
		try {
			...
			if ( ! is_readable($outputFilename) || ($output = file_get_contents($outputFilename)) === FALSE) {
				throw new \CI\Exception('Nepodařilo se dohledat výstup kontroly Cypressu');
			}
			...
			$cypress = new \CI\Cypress\Cypress($output);
			...
			$this->statusPublicator->publish($repository, $currentCommit, $cypress);
		} catch (\Exception $e) {
			$this->logger->addError($e);
		} finally {
			if (file_exists($instancePath . '/output.cypress')) {
				try {
					\Nette\Utils\FileSystem::delete($instancePath . '/output.cypress');
